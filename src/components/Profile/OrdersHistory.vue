...
<template>
  <div class="container mx-auto p-4">
    ...
    <template v-if="!loading">
      ...
      <ul class="space-y-3">
        <li v-for="order in orders" :key="order.id" class="flex items-center p-4 bg-white drop-shadow-lg rounded-md">
          <img :src="order.images" alt="Product Image" class="w-36 h-36 object-cover mr-4 rounded-md">
          <div>
            <div class="text-lg font-semibold -mt-1">{{ order.name }}</div>
            <div class="text-gray-600 mt-16">Total price: {{ order.total_price }}$</div>
          </div>
